...
def run():
    ...
    inputTensorList = []
    inputTensorList.append( network.add_input('inputT', trt.float16, [-1,-1,nEmbedding]) )
    ...
    bufferH = []
    bufferH.append( np.random.rand(nBS,nSL,nEmbedding).astype(np.float16).reshape(nBS,nSL,nEmbedding) * 2 - 1)
